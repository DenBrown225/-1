...
root.geometry("750x600")  # Устанавливаем размеры окна
customtkinter.set_default_color_theme("blue")  # Устанавливаем цветовую тему
customtkinter.set_appearance_mode("light")  # Устанавливаем режим отображения

# Позиции для размещения элементов интерфейса
a = 110
b = -45
a1 = 325
a2 = 714
...
# Создаем канвас для рисования прямоугольников
canvas = tk.Canvas(root, bg="grey92", width=750, height=150, highlightthickness=0)
canvas.pack(anchor="n")  # Привязываем канвас к верхней части окна

# Создаем слайдер для выбора значений от 0 до 100
slider = customtkinter.CTkSlider(master=root, from_=0, to=100, command=w)
slider.place(x=a, y=b)  # Размещаем слайдер на окне
slider.set(0)  # Устанавливаем начальное значение слайдера

# Создаем кнопки с различными функциями и размещаем их на окне
button2 = ctk.CTkButton(root, text="🔈", command=sound, width=26, height=25, fg_color="dark grey", hover_color="grey")
button2.place(x=a1, y=b1)  # Кнопка для звука

button34 = ctk.CTkButton(root, text="▶", command=not_say, width=25, height=25, fg_color="dark grey", hover_color="grey")
button34.place(x=a3, y=b3)  # Кнопка для воспроизведения/паузы

button3 = ctk.CTkButton(root, text="?", command=documentation, width=25, height=25, fg_color="dark grey", hover_color="grey")
button3.place(x=a2, y=b2)  # Кнопка для справки

# Создаем прямоугольники на канвасе для визуализации (например: индикаторы состояния)
rectangle_y = -150  # Начальная позиция Y первого прямоугольника
rectangle = canvas.create_rectangle(750, rectangle_y, 0, rectangle_y + 149, fill="light grey")

rectangle_y2 = -51  # Начальная позиция Y второго прямоугольника
rectangle2 = canvas.create_rectangle(390, rectangle_y2, 100, rectangle_y2 + 28, fill="grey93")

rectangle_y3 = -141  # Начальная позиция Y третьего прямоугольника
...
# Настройка шрифта для кнопки переключения состояния прямоугольника
custom_font = ("Helvetica", 16)
button54 = ctk.CTkButton(root, text="★", command=toggle_rectangle,
                          width=30, height=30,
                          fg_color="grey96", hover_color="grey100",
                          text_color="red", text_color_disabled="red",
                          font=custom_font)
button54.place(x=a4, y=b4)  # Кнопка для переключения состояния прямоугольника

# Текстовый виджет для вывода ответа функции gbt()
text_widget = ctk.CTkTextbox(root, width=505, height=300)
text_widget.place(x=65, y=250)  # Размещаем текстовый виджет на окне

# Метка для помощи в интерфейсе (инструкции пользователю)
label2 = ctk.CTkLabel(root,
                       text="Вызов помошника \n \n                 O \n \n Анализ речи \n \n                 o \n \n Нейросеть \n \n                 o \n",
                       text_color="black")
label2.place(x=605,y=200)  # Размещаем метку на окне

# Поле ввода вопросов от пользователя
entry = ctk.CTkEntry(root,
                     placeholder_text="Задайте вопрос",
                     width=479,
                     height=25)
entry.place(x=65,y=184)  # Размещаем поле ввода на окне

# Кнопка для отправки запросов (ввод текста из поля entry)
button = ctk.CTkButton(root,
                        ...
                        fg_color="dark grey",
                        hover_color="grey")
button.place(x=544,y=184)  # Размещаем кнопку на окне

# Отключаем кнопку button34 по умолчанию (например: если функция не доступна)
button34.configure(state='disabled')

# Запускаем выполнение функций в отдельных потоках (например: работа с нейросетью)
threading.Thread(target=run_jarvis, daemon=True).start()

# Запуск главного цикла приложения (ожидание событий от пользователя)
root.mainloop()
